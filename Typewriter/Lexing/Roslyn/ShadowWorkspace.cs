...
namespace Typewriter.Lexing.Roslyn
{
    internal class ShadowWorkspace : Workspace
    {
        ...
        public DocumentId AddProjectWithDocument(string documentFileName, string text)
        {
            ...
            var name = Path.GetFileNameWithoutExtension(documentFileName) + _counter++;

            var projectId = ProjectId.CreateNewId();
            _defaultMetadataReferences = DefaultReferences.Select(CreateReference).ToList();

            var asm = Assembly.Load("netstandard, Version=2.0.0.0, Culture=neutral, PublicKeyToken=cc7b13ffcd2ddd51");
            _defaultMetadataReferences.Add(CreateReference(asm));
            ...
            var projectInfo = ProjectInfo.Create(projectId, new VersionStamp(), name, name + ".dll", LanguageNames.CSharp, metadataReferences: _defaultMetadataReferences, compilationOptions: new CSharpCompilationOptions(OutputKind.DynamicallyLinkedLibrary));
        ...
        public void SetMetadataReferences(DocumentId documentId, IEnumerable<Assembly> references)
        {
            ...
            var newReferences = references.Select(CreateReference).Union(_defaultMetadataReferences, MetadataReferenceComparer).ToList();
